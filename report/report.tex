...
We are asked to bound the probability of a false answer to $\frac{1}{n}$. As we are substituting all $d$ polynomial variables into random numbers, we are essentially evaluating a polynomial of degree 1. From Lemma~\ref{lma:zippelSchwarz} it follows that the probability for a false answer for $\rvert S \rvert = p$ is $\frac{1}{p}$. By setting $p \geq n$ we ensure the desired probability $\frac{1}{n}$ for our algorithm.
